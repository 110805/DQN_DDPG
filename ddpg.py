...
class GaussianNoise:
    ...
    def sample(self):
        return torch.from_numpy(np.random.normal(self.mu, self.std)).cuda()
...
def test(args, env, agent, writer):
    ...
    for n_episode, seed in enumerate(seeds):
        ...
        for t in itertools.count(start=1):
            # select action
            state = torch.from_numpy(state).to(args.device)
            action = agent.select_action(state, noise=False) # When testing DDPG, action selection need NOT include the noise
            # execute action
            next_state, reward, done, _ = env.step(action)
